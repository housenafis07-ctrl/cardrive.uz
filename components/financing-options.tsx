...
export type FinancingProgram = {
  ...
  insurance_type:string|null; insurance_amount:number|null; insurance_percent:number|null; benefits_uz:string|null; benefits_ru:string|null;
  banks:Bank|null;
};
...
export function FinancingOptions({carPrice,currency,programs,selectedProgramId=null,onSelectProgram}:{carPrice:number;currency:string;programs:FinancingProgram[];selectedProgramId?:string|null;onSelectProgram?:(id:string)=>void}){
 ...
 return <div className="space-y-4"><div><p className="text-lg font-black">Kredit va bo&apos;lib to&apos;lash variantlari</p><p className="mt-1 text-sm text-slate-500">Kerakli bank yoki diler variantini tanlang. Boshlang&apos;ich to&apos;lov va muddatni o&apos;zgartirsangiz, oylik to&apos;lov avtomatik qayta hisoblanadi.</p></div>{programs.map(program=>{const selectable=program.financing_type==="installment"||program.banks?.integration_status==="connected";return <div key={program.id}><div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-500"><span>{LABEL[program.financing_type]??program.financing_type}</span>{!selectable&&<span className="rounded-full bg-amber-50 px-2 py-1 text-amber-700">Ma&apos;lumot uchun</span>}</div><FinancingCalculator carPrice={carPrice} currency={currency} program={program} selected={selectedProgramId===program.id} selectable={selectable} onSelect={()=>selectable&&onSelectProgram?.(program.id)}/></div>})}</div>;
}

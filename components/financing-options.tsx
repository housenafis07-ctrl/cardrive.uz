...
const LABEL={uz:{credit:"Kredit",installment:"Bo'lib to'lash",promotional:"Aksiya"},ru:{credit:"Кредит",installment:"Рассрочка",promotional:"Акция"}} as const;
...
export function FinancingOptions({carPrice,currency,programs,selectedProgramId=null,onSelectProgram,purchaseType="credit",locale="uz"}:{carPrice:number;currency:string;programs:FinancingProgram[];selectedProgramId?:string|null;onSelectProgram?:(id:string)=>void;purchaseType:"credit"|"installment";locale?:"uz"|"ru"}){
 ...
 return <div className="space-y-4"><div><p className="text-lg font-black">{locale==="ru"?"Варианты кредита и рассрочки":"Kredit va bo&apos;lib to&apos;lash variantlari"}</p><p className="mt-1 text-sm text-slate-500">{locale==="ru"?"Для выбранного автомобиля показаны только комбинации первоначального взноса и срока, указанные банком или дилером.":"Tanlangan avtomobil uchun faqat bank yoki diler belgilagan boshlang&apos;ich to&apos;lov va muddat kombinatsiyalari ko&apos;rsatiladi."}</p></div>{visiblePrograms.map(program=><div key={program.id}><div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-500"><span>{LABEL[locale][program.financing_type as keyof typeof LABEL[typeof locale]]??program.financing_type}</span></div><FinancingCalculator carPrice={carPrice} currency={currency} program={program} selected={selectedProgramId===program.id} selectable={true} onSelect={()=>onSelectProgram?.(program.id)} locale={locale}/></div>)}</div>;
}

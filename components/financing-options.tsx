...
export function FinancingOptions({carPrice,currency,programs,selectedProgramId=null,onSelectProgram}:{carPrice:number;currency:string;programs:FinancingProgram[];selectedProgramId?:string|null;onSelectProgram?:(id:string)=>void}){
 ...
 return <div className="space-y-4"><div><p className="text-lg font-black">Kredit va bo&apos;lib to&apos;lash variantlari</p><p className="mt-1 text-sm text-slate-500">Kerakli bank yoki diler variantini tanlang. Boshlang&apos;ich to&apos;lov va muddatni o&apos;zgartirsangiz, oylik to&apos;lov avtomatik qayta hisoblanadi.</p></div>{programs.map(program=><div key={program.id}><div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-slate-500"><span>{LABEL[program.financing_type]??program.financing_type}</span></div><FinancingCalculator carPrice={carPrice} currency={currency} program={program} selected={selectedProgramId===program.id} selectable={true} onSelect={()=>onSelectProgram?.(program.id)}/></div>)}</div>;
}

...
function decode(value: string) {
  ...
  let phone: string;
  try { phone = Buffer.from(encodedPhone, "base64url").toString("utf8"); } catch { return null; }
  const expected = sign(phone); const a = Buffer.from(providedSignature); const b = Buffer.from(expected);
  if (a.length !== b.length || !timingSafeEqual(a, b)) return null;
  return phone;
}

async function setSession(phone: string) {
  const store = await cookies();
  store.set(COOKIE_NAME, encode(phone), { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: MAX_AGE });
}

export async function establishCustomerSession(phone: string) {
  const normalized = phone.trim();
  if (!normalized) throw new Error("Telefon raqami yaroqsiz");
  const result = await createServiceRoleClient().from("profiles").select("id,phone,is_active").eq("phone", normalized).maybeSingle();
  if (result.error) throw new Error("Mijoz ma'lumotlarini tekshirib bo'lmadi");
  if (!result.data || result.data.is_active === false) throw new Error("Bu telefon raqami bilan mijoz topilmadi");
...
export async function registerCustomer(phone: string, fullName: string) {
  ...
  const created = await db.from("profiles").insert({ phone: normalized, full_name: name, role: "customer", is_active: true }).select("id,phone").single();
  if (created.error || !created.data) throw new Error("Mijozni ro‘yxatdan o‘tkazib bo‘lmadi");
  await setSession(normalized);
}

export async function getCurrentCustomer(): Promise<Customer | null> {
  const store = await cookies(); const value = store.get(COOKIE_NAME)?.value; if (!value) return null;
  const phone = decode(value); if (!phone) return null;
  const result = await createServiceRoleClient().from("profiles").select("id,phone,is_active,full_name").eq("phone", phone).maybeSingle();
  if (result.error || !result.data || result.data.is_active === false) return null;
  return { id: result.data.id, phone: result.data.phone, full_name: result.data.full_name };
}

...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { type: "website", title, description, url: `https://cardrive.uz/cars/${slug}`, siteName: "Cardrive.uz" },
...
export default async function CarDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const specs = [
    ...
    ["Batareya sig‘imi", car.battery_capacity_kwh !== null && car.battery_capacity_kwh !== undefined ? `${car.battery_capacity_kwh} kWh` : null],
    ["Elektr yurish masofasi", car.range_km ? `${car.range_km} km` : null], ["0–100 km/soat", car.acceleration_0_100_sec ? `${car.acceleration_0_100_sec} s` : null],
    ["Tezkor zaryadlash", car.charging_time_minutes ? `${car.charging_time_minutes} min` : null], [x.seats, car.seats],
  ...
  const jivoFinancing = financingPrograms.map((program) => ({
    bankName: program.banks?.name ?? "Diler",
    programName: program.name,
    interest: program.annual_interest_rate,
    minDown: program.min_down_payment_percent,
    maxTerm: program.max_term_months,
  }));
  const structuredData = {
    "@context": "https://schema.org",
    "@type": "Product",
    name: `${brandName} ${modelName} ${car.name}`.trim(),
    description: car.description ?? `${brandName} ${modelName} ${car.name} ${car.year}`,
    url: pageUrl,
    image: imageUrl ? [imageUrl] : undefined,
    brand: { "@type": "Brand", name: brandName },
    model: modelName,
    offers: { "@type": "Offer", url: pageUrl, priceCurrency: car.currency, price: Number(car.price), availability: car.stock_status === "available" ? "https://schema.org/InStock" : "https://schema.org/OutOfStock" },
  };

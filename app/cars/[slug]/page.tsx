...
export default async function CarDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const specs = [
    ...
    [x.drive, car.drive_type],
    [x.engine, car.engine_volume ? `${car.engine_volume} L` : null],
    [x.power, car.engine_power ? `${car.engine_power} ${locale === "ru" ? "л.с." : "ot kuchi"}` : null],
    [x.range, car.range_km ? `${car.range_km} km` : null],
    [x.seats, car.seats],
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-7xl px-5 py-8">
        <Link href="/cars" className="text-sm font-bold underline">← {x.back}</Link>
        <div className="mt-6 grid gap-8 lg:grid-cols-[1.2fr_.8fr]">
          <section>
            <CarColorGallery images={car.car_images ?? []} carName={car.name} locale={locale} />
          </section>
          <section className="lg:sticky lg:top-5 lg:h-fit">
            <div className="flex items-center gap-2">
              {car.brands?.logo_url ? (
                <img src={car.brands.logo_url} alt={car.brands?.name ?? ""} className="h-6 w-6 object-contain" />
              ) : (
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              {car.stock_status === "available" ? <OrderFlow carId={car.id} carName={car.name} price={car.price} currency={car.currency} isAuthenticated={!!customer} financingPrograms={financingPrograms} locale={locale} /> : <button type="button" disabled className="rounded-full bg-slate-300 px-5 py-3 font-bold text-slate-600">{x.unavailable}</button>}
              <button type="button" className="rounded-full border border-slate-300 px-5 py-3 font-bold">{x.advice}</button>
            </div>
          </section>
        </div>
        {car.description && <section className="mt-12 max-w-3xl"><h2 className="text-2xl font-black">{x.description}</h2><p className="mt-3 whitespace-pre-wrap leading-7 text-slate-700">{car.description}</p></section>}
        <section className="mt-12"><h2 className="text-2xl font-black">{x.specs}</h2><dl className="mt-5 grid gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200 sm:grid-cols-2">{specs.filter(([, value]) => value !== null && value !== undefined && value !== "").map(([label, value]) => <div key={label} className="flex justify-between gap-4 bg-white p-4"><dt className="text-slate-500">{label}</dt><dd className="font-semibold">{String(value)}</dd></div>)}</dl></section>
      </main>
      <Footer />
    </>
  );

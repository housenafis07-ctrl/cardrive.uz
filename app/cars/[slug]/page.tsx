...
export default async function CarDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const specs = [[x.body, car.body_type], [x.fuel, car.fuel_type], [x.transmission, car.transmission], [x.drive, car.drive_type], [x.engine, engineLabel], [x.power, car.engine_power ? `${car.engine_power} ${locale === "ru" ? "л.с." : "ot kuchi"}` : null], ["Batareya sig‘imi", car.battery_capacity_kwh !== null && car.battery_capacity_kwh !== undefined ? `${car.battery_capacity_kwh} kWh` : null], ["Elektr yurish masofasi", car.range_km ? `${car.range_km} km` : null], ["0–100 km/soat", car.acceleration_0_100_sec ? `${car.acceleration_0_100_sec} s` : null], ["Tezkor zaryadlash", car.charging_time_minutes ? `${car.charging_time_minutes} min` : null], [x.seats, car.seats], [x.color, locale === "ru" ? localizedCar.color_name_ru ?? car.color : localizedCar.color_name_uz ?? car.color]];

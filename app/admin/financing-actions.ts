...
export async function createBankAction(d: FormData) {
  ...
  const name = value(d, "name"); const code = value(d, "code");
...
export async function createFinancingProgramAction(d: FormData) {
  const user = await requireAdminUser();
  const type = value(d,"financingType"); const name = value(d,"name");
  const bankId = value(d,"bankId");
  ...
  const r = await c.from("financing_programs").insert({
    bank_id: bankId,
    name, name_uz: value(d,"nameUz") ?? name, name_ru: value(d,"nameRu"), type,
    interest_rate: num(d,"annualInterestRate") ?? 0,
    down_payment_percent: num(d,"minDownPaymentPercent") ?? 0,
    term_months: num(d,"maxTermMonths") ?? num(d,"minTermMonths") ?? 12,
    commission_percent: num(d,"commissionPercent") ?? 0,
    min_car_price: num(d,"minCarPrice"), max_car_price: num(d,"maxCarPrice"),
    min_amount: num(d,"minAmount"), max_amount: num(d,"maxAmount"),
    max_financing_percent: num(d,"maxFinancingPercent"),
    insurance_type: value(d,"insuranceType"), insurance_amount: num(d,"insuranceAmount"), insurance_percent: num(d,"insurancePercent"),
    benefits_uz: value(d,"benefitsUz"), benefits_ru: value(d,"benefitsRu"),
    description: value(d,"description"), eligibility_notes: value(d,"eligibilityNotes"),
    currency: value(d,"currency") ?? "UZS", is_active: d.get("isActive") === "on", sort_order: num(d,"displayOrder") ?? 0,
  ...
  if (dealerIds.length) { const links = await c.from("financing_program_dealers").insert(dealerIds.map(dealer_id => ({ dealer_id, financing_program_id:r.data.id }))); if (links.error) redirect(`/admin/financing-programs?error=${encodeURIComponent(links.error.message)}`); }
  const carIds = selected(d,"carIds");
  if (carIds.length) { const links = await c.from("financing_program_cars").insert(carIds.map(car_id => ({ car_id, financing_program_id:r.data.id }))); if (links.error) redirect(`/admin/financing-programs?error=${encodeURIComponent(links.error.message)}`); }

...
export default async function FinancingProgramsPage() {
  ...
  const [programs, banks, dealers, cars] = await Promise.all([
    c.from("financing_programs").select("id,bank_id,name,type,interest_rate,down_payment_percent,term_months,insurance_amount,insurance_percent,benefits_uz,is_active").order("sort_order").order("name"),
  ...
  return <div>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_480px]">
      <AdminTable>
        <thead className="border-b bg-slate-50"><tr><th className="p-3">Bank/Diler</th><th className="p-3">Dastur</th><th className="p-3">Turi</th><th className="p-3">Avans</th><th className="p-3">Foiz</th><th className="p-3">Muddat</th><th className="p-3">Sug‘urta</th></tr></thead>
        <tbody>{(programs.data ?? []).map(x => <tr key={x.id} className="border-b">
          <td className="p-3">{x.bank_id ? bankMap.get(x.bank_id) ?? "—" : "Diler hisobidan"}</td>
          <td className="p-3 font-semibold">{x.name}</td><td className="p-3">{x.type === "credit" ? "Kredit" : "Rassrochka"}</td>
          <td className="p-3">{x.down_payment_percent ?? 0}%</td><td className="p-3">{x.interest_rate ?? 0}%</td><td className="p-3">{x.term_months ?? "—"} oy</td>
          <td className="p-3">{x.insurance_amount ? `${Number(x.insurance_amount).toLocaleString()} UZS` : x.insurance_percent ? `${x.insurance_percent}%` : "—"}</td>
        </tr>)}</tbody>
      </AdminTable>
      ...
      <form action={createFinancingProgramAction} className="space-y-4 rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <div className="grid gap-3 sm:grid-cols-2"><Field name="minDownPaymentPercent" label="Boshlang‘ich to‘lov %" type="number"/><Field name="annualInterestRate" label="Yillik foiz %" type="number"/><Field name="minTermMonths" label="Min. muddat, oy" type="number"/><Field name="maxTermMonths" label="Max. muddat, oy" type="number"/><Field name="maxFinancingPercent" label="Max. moliyalashtirish %" type="number"/><Field name="commissionPercent" label="Komissiya %" type="number"/></div>
        ...
        <label className="block text-sm font-semibold">Dilerlar<select name="dealerIds" multiple className="mt-1 h-28 w-full rounded border p-2">{(dealers.data ?? []).map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select><span className="text-xs text-slate-500">Rassrochkani qaysi dilerlar taklif qilishini tanlang.</span></label>
        ...
        <label className="block text-sm font-semibold">Izoh<textarea name="description" className="mt-1 w-full rounded border p-3"/></label>
        <label className="block text-sm font-semibold">Talablar<textarea name="eligibilityNotes" className="mt-1 w-full rounded border p-3"/></label>
        <div className="grid gap-3 sm:grid-cols-2"><Field name="sourceLabel" label="Manba nomi"/><Field name="sourceUrl" label="Manba URL"/></div>

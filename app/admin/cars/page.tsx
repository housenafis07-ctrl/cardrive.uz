      ...
      <div className="mt-6 grid gap-6 2xl:grid-cols-[1fr_420px]">
            ...
            <AdminTable>
              <thead className="border-b bg-slate-50">
                <tr>
                  ...
                  <th className="p-3">Kategoriyalar</th>
                ...
                {result.data.map((car) => (
                  <tr key={car.id} className="border-b last:border-0">
                    ...
                    <td className="p-3 text-xs">
                      {car.is_premium ? "💎 Premium " : ""}
                      {car.is_commercial ? "🚚 Tijorat" : ""}
                      {!car.is_premium && !car.is_commercial ? "—" : ""}
                    </td>
                    ...
                    <td className="p-3">
                      <form action={deactivateAction}>
                        ...
                        <div className="flex flex-wrap gap-3">
                          <Link className="text-sm font-bold text-slate-900 underline" href={`/admin/cars/${car.id}/edit`}>Tahrirlash</Link>
                          <Link className="text-sm font-bold text-slate-900 underline" href={`/admin/cars/${car.id}/images`}>Rasmlar · ranglar</Link>
        ...
        <section>
          ...
          <form action={createCarAction} className="grid gap-4 rounded-2xl border bg-white p-5 shadow-sm sm:grid-cols-2">
            ...
            <label className="text-sm font-semibold">Brend
            ...
            <label className="text-sm font-semibold">Model
              <select name="modelId" required className="mt-1 w-full rounded-lg border p-2">
                <option value="">Tanlang</option>
                {models.data?.map((model) => <option key={model.id} value={model.id}>{model.brands?.name ? `${model.brands.name} — ` : ""}{model.name}</option>)}
              </select>
            </label>

            <label className="text-sm font-semibold">Diler
              <select name="dealerId" className="mt-1 w-full rounded-lg border p-2">
                <option value="">Belgilanmagan</option>
                {(dealers.data ?? []).map((dealer) => <option key={dealer.id} value={dealer.id}>{dealer.name}</option>)}
              </select>
            </label>
            ...
            <label className="text-sm font-semibold">Klassifikatsiya
              <div className="mt-2 flex flex-col gap-2 font-medium">
                <span><input type="checkbox" name="isFeatured" className="mr-2" />🔥 Mashhur / Tavsiya</span>
                <span><input type="checkbox" name="isPremium" className="mr-2" />💎 Premium</span>
                <span><input type="checkbox" name="isCommercial" className="mr-2" />🚚 Tijorat</span>
              </div>
            </label>

            <label className="text-sm font-semibold">Zaxira holati
            ...
            <label className="col-span-full text-sm font-semibold">Tavsif
              <textarea name="description" className="mt-1 w-full rounded-lg border p-3" />
            </label>
            <label className="col-span-full text-sm font-semibold">Qisqa tavsif
            ...
            <label className="col-span-full text-sm font-semibold">
              <input type="checkbox" name="isActive" defaultChecked className="mr-2" />Faol
            </label>

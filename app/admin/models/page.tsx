...
export default async function ModelsPage({ searchParams }: { searchParams: Promise<{ q?: string; brand?: string; page?: string; error?: string }> }) {
    ...
    <div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_360px]">
        <section>
          {result.data?.length ? (
            <AdminTable>
              <thead className="border-b bg-slate-50"><tr><th className="p-3">Nomi</th><th className="p-3">Brend</th><th className="p-3">Holat</th><th className="p-3" /></tr></thead>
              <tbody>
                {result.data.map((model) => (
                  <tr key={model.id} className="border-b last:border-0"><td className="p-3 font-semibold">{model.name}</td><td className="p-3">{model.brands?.name}</td><td className="p-3">{model.is_active ? "Faol" : "Nofaol"}</td><td className="p-3"><form action={deactivateAction}><input type="hidden" name="kind" value="car_models" /><input type="hidden" name="id" value={model.id} /><div className="flex gap-3"><Link className="text-sm font-bold text-slate-900 underline" href={`/admin/models/${model.id}/edit`}>Tahrirlash</Link><button className="text-sm font-bold text-red-700">Faolsizlantirish</button></div></form></td></tr>
                ))}
              </tbody>
            </AdminTable>
        ...
        <form action={createModelAction} className="space-y-4 rounded-2xl border bg-white p-5 shadow-sm">
          ...
          <label className="block text-sm font-semibold">Brend<select name="brandId" required className="mt-1 w-full rounded-lg border p-2"><option value="">Tanlang</option>{brands.data?.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label>
          <Field name="name" label="Nomi" required />
          <p className="text-xs text-slate-500">Slug model nomidan avtomatik yaratiladi.</p>

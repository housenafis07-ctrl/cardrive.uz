              ...
              <tbody>
                {result.data.map((brand) => (
                  <tr key={brand.id} className="border-b last:border-0">
                    ...
                    <td className="p-3 text-slate-600">{brand.slug}</td>
                    <td className="p-3">{brand.is_active ? "Faol" : "Nofaol"}</td>
                    <td className="p-3">
                      <form action={deactivateAction}>
                        <input type="hidden" name="kind" value="brands" />
                        <input type="hidden" name="id" value={brand.id} />
                        <div className="flex gap-3">
                          <Link className="text-sm font-bold text-slate-900 underline" href={`/admin/brands/${brand.id}/edit`}>
                            Tahrirlash
                          </Link>

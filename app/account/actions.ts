...
import { establishCustomerSession, registerCustomer, getCurrentCustomer } from "@/lib/customer-session";
...
export type VerifyOtpResult = { status: "verified" | "new_customer"; message?: string } | { status: "error"; message: string };
export type RegisterCustomerResult = { status: "registered" } | { status: "error"; message: string };
...
export async function verifyCustomerOtpAction(phone: string, code: string, purpose: OtpPurpose): Promise<VerifyOtpResult> { try { await new OtpService(new SmsService(smsProvider)).verifyOtp({ phone, code, purpose }); const customer = await getCurrentCustomer(); if (customer) { await establishCustomerSession(phone); revalidatePath("/account/orders"); return { status: "verified" }; } return { status: "new_customer", message: "Yangi mijoz uchun ma'lumotlaringizni kiriting" }; } catch (error) { return { status: "error", message: error instanceof Error ? error.message : "Kodni tasdiqlab bo'lmadi" }; } }
export async function registerCustomerAction(phone: string, fullName: string): Promise<RegisterCustomerResult> { try { await registerCustomer(phone, fullName); revalidatePath("/account/orders"); return { status: "registered" }; } catch (error) { return { status: "error", message: error instanceof Error ? error.message : "Ro‘yxatdan o‘tib bo‘lmadi" }; } }

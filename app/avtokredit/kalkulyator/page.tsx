...
type RawCalculatorCar = {
  ...
  currency: string;
  year: number | string;
  brands?: NamedRelation | NamedRelation[] | null;
  car_models?: NamedRelation | NamedRelation[] | null;
};
type CalculatorCar = {
  id: string;
  slug: string;
...
type RawFinancingProgram = {
  id: string;
  name?: string | null;
  name_uz?: string | null;
  name_ru?: string | null;
  annual_interest_rate?: number | string | null;
  min_down_payment_percent?: number | string | null;
  max_term_months?: number | string | null;
  min_term_months?: number | string | null;
  currency?: string | null;
  banks?: NamedRelation & { name_ru?: string | null } | null;
  financing_program_rules?: FinancingRule[] | null;
};
...
export default async function CreditCalculatorPage({ searchParams }: { searchParams?: Promise<Params> }) {
  ...
  const rawCars = (carsResult.data ?? []) as RawCalculatorCar[];
  const cars: CalculatorCar[] = rawCars.map((car) => {
  ...
  }).sort((a, b) => a.brandName.localeCompare(b.brandName, locale) || a.modelName.localeCompare(b.modelName, locale) || a.name.localeCompare(b.name, locale));
  const selectedCar = params.carId ? cars.find((car) => car.id === params.carId) ?? null : null;
  const programsResult = selectedCar ? await new FinancingService().getApplicableProgramsForCar(selectedCar.id) : null;
  const rawPrograms = (programsResult?.data ?? []) as RawFinancingProgram[];
  const programs: CalculatorProgram[] = rawPrograms.map((program) => {
    const rules = Array.isArray(program.financing_program_rules)
      ? program.financing_program_rules.filter((rule) => rule.is_available !== false)
      : [];
    const rule = rules.slice().sort((a, b) => Number(a.annual_interest_rate) - Number(b.annual_interest_rate) || Number(a.down_payment_percent) - Number(b.down_payment_percent) || Number(b.term_months) - Number(a.term_months))[0];
    return {
      id: program.id,
      name: ru ? program.name_ru ?? program.name ?? "" : program.name_uz ?? program.name ?? "",
      bankName: ru ? program.banks?.name_ru ?? program.banks?.name ?? "Diler" : program.banks?.name ?? "Diler",

...
export class CatalogRepository {
  async getCars(filters: CatalogQuery) {
    ...
    let query = createPublicServerClient().from("cars").select("id,name,slug,price,currency,old_price,year,body_type,fuel_type,transmission,drive_type,stock_status,is_featured,brands(name,slug),car_models(name,slug),car_images(public_url,alt_text,is_primary,sort_order)", { count: "exact" }).eq("is_active", true).in("stock_status", filters.status ? [filters.status] : ["available", "coming_soon"]);
    if (filters.brand) query = query.eq("brands.slug", filters.brand); if (filters.model) query = query.eq("car_models.slug", filters.model); if (filters.body) query = query.eq("body_type", filters.body); if (filters.fuel) query = query.eq("fuel_type", filters.fuel); if (filters.transmission) query = query.eq("transmission", filters.transmission); if (filters.drive) query = query.eq("drive_type", filters.drive); if (filters.minPrice !== undefined) query = query.gte("price", filters.minPrice); if (filters.maxPrice !== undefined) query = query.lte("price", filters.maxPrice); if (filters.minYear !== undefined) query = query.gte("year", filters.minYear); if (filters.maxYear !== undefined) query = query.lte("year", filters.maxYear); if (filters.q) query = query.ilike("name", "%" + filters.q.replace(/[,%()]/g, "") + "%");
    ...
    const result = await query.range(from, from + PAGE_SIZE - 1); return { ...result, data: normalizeCars(result.data) };
  }
  async getCarById(id: string) { const result = await createPublicServerClient().from("cars").select("id,name,slug,price,currency,stock_status,is_active,brands(name,slug),car_models(name,slug),car_images(public_url,alt_text,is_primary,sort_order)").eq("id", id).eq("is_active", true).single(); return { ...result, data: result.data ? normalizeCar(result.data) : null }; }
  async getCarBySlug(slug: string) { const result = await createPublicServerClient().from("cars").select("*,brands(name,slug),car_models(name,slug),car_images(public_url,alt_text,is_primary,sort_order)").eq("slug", slug).eq("is_active", true).single(); return { ...result, data: result.data ? normalizeCar(result.data) : null }; }
  async getFeaturedCars(limit = 4) { const result = await createPublicServerClient().from("cars").select("id,name,slug,price,currency,year,stock_status,is_featured,brands(name),car_models(name),car_images(public_url,alt_text,is_primary,sort_order)").eq("is_active", true).eq("is_featured", true).order("created_at", { ascending: false }).limit(limit); return { ...result, data: normalizeCars(result.data) }; }
  async getAvailableCars(limit = 4) { const result = await createPublicServerClient().from("cars").select("id,name,slug,price,currency,year,stock_status,is_featured,brands(name),car_models(name),car_images(public_url,alt_text,is_primary,sort_order)").eq("is_active", true).eq("stock_status", "available").order("created_at", { ascending: false }).limit(limit); return { ...result, data: normalizeCars(result.data) }; }

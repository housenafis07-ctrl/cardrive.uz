...
export class CatalogRepository {
  ...
  async getFeaturedCars(limit = 4) { const result = await createPublicServerClient().from("cars").select("id,name,slug,price,currency,year,body_type,fuel_type,transmission,engine_volume,stock_status,is_featured,brands(name,logo_url),car_models(name),car_images(public_url,alt_text,is_primary,sort_order,color_id,car_colors(id,name_uz,name_ru,hex_code))").eq("is_active", true).eq("is_featured", true).order("created_at", { ascending: false }).limit(limit); return { ...result, data: await this.enrichFinancing(result.data) }; }
  async getAvailableCars(limit = 4) { const result = await createPublicServerClient().from("cars").select("id,name,slug,price,currency,year,body_type,fuel_type,transmission,engine_volume,stock_status,is_featured,brands(name,logo_url),car_models(name),car_images(public_url,alt_text,is_primary,sort_order,color_id,car_colors(id,name_uz,name_ru,hex_code))").eq("is_active", true).eq("stock_status", "available").order("created_at", { ascending: false }).limit(limit); return { ...result, data: await this.enrichFinancing(result.data) }; }
  async getActiveBrands() {
    ...
    for (const model of modelsResult.data ?? []) if (model.brand_id) modelCounts.set(model.brand_id, (modelCounts.get(model.brand_id) ?? 0) + 1);
    return { ...brandsResult, data: brandsResult.data?.map((brand) => ({ ...brand, model_count: modelCounts.get(brand.id) ?? 0 })) ?? null };
  }

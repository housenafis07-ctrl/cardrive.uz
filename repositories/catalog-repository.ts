...
function monthlyPayment(principal: number, annualRate: number, months: number) { if (principal <= 0 || months <= 0) return 0; const r = annualRate / 100 / 12; return r === 0 ? Math.round(principal / months) : Math.round((principal * r) / (1 - Math.pow(1 + r, -months))); }
...
export class CatalogRepository {
  ...
  private async enrichFinancing<T extends CarRelations>(cars: T[] | null) {
    if (!cars?.length) return [] as Array<NormalizedCar<T>>;
    return Promise.all(cars.map(async (car) => {
      const programs = await this.financing.getApplicableProgramsForCar(car.id);
      const offers = (programs.data ?? []).map((p) => {
        const months = Math.max(1, Number(p.max_term_months ?? p.term_months ?? 60));
        const down = Math.min(100, Math.max(0, Number(p.min_down_payment_percent ?? p.down_payment_percent ?? 0)));
        const rate = Math.max(0, Number(p.annual_interest_rate ?? p.interest_rate ?? 0));
        const payment = monthlyPayment(Number(car.price) * (1 - down / 100), rate, months);
        return { monthlyPayment: payment, downPaymentPercent: down, termMonths: months, providerName: p.banks?.name ?? p.dealers?.name ?? p.name ?? "Moliyalashtirish", financingType: financingType(p.financing_type ?? p.type), interestRate: rate };
      }).filter((o) => o.monthlyPayment > 0).sort((a, b) => a.monthlyPayment - b.monthlyPayment || a.termMonths - b.termMonths);
  ...
  async getCars(filters: CatalogQuery) {
    ...
    if (filters.category) { const category = await client.from("car_categories").select("id").eq("key", filters.category).eq("is_active", true).maybeSingle(); if (category.error) return { ...category, data: null }; if (!category.data) return { data: [], count: 0, error: null }; const items = await client.from("car_category_items").select("car_id").eq("category_id", category.data.id); if (items.error) return { ...items, data: null }; const ids = (items.data ?? []).map((i) => i.car_id); if (!ids.length) return { data: [], count: 0, error: null }; query = query.in("id", ids); }
    if (filters.brand) query = query.eq("brands.slug", filters.brand); if (filters.model) query = query.eq("car_models.slug", filters.model); if (filters.body) query = query.eq("body_type", filters.body); if (filters.fuel) query = query.eq("fuel_type", filters.fuel); if (filters.transmission) query = query.eq("transmission", filters.transmission); if (filters.drive) query = query.eq("drive_type", filters.drive); if (filters.minPrice !== undefined) query = query.gte("price", filters.minPrice); if (filters.maxPrice !== undefined) query = query.lte("price", filters.maxPrice); if (filters.minYear !== undefined) query = query.gte("year", filters.minYear); if (filters.maxYear !== undefined) query = query.lte("year", filters.maxYear);
    ...
    if (filters.q) { const result = await query; if (result.error) return { ...result, data: null }; const enriched = await this.enrichFinancing(result.data); const ranked = enriched.map((car) => ({ car, score: searchRelevance(filters.q!, [car.name, car.brands?.name ?? "", car.car_models?.name ?? "", car.slug]) })).filter((x) => x.score > 0).sort((a, b) => b.score - a.score).map((x) => x.car); return { data: ranked.slice(from, from + PAGE_SIZE), count: ranked.length, error: null }; }
    const result = await query.range(from, from + PAGE_SIZE - 1); return { ...result, data: await this.enrichFinancing(result.data) };
  }

...
export class CatalogRepository {
  async getCars(filters: CatalogQuery) {
    ...
    let query = createPublicServerClient().from("cars").select("id,name,slug,price,currency,old_price,year,body_type,fuel_type,transmission,drive_type,stock_status,is_featured,brands(name,slug,logo_url),car_models(name,slug),car_images(public_url,alt_text,is_primary,sort_order)", { count: "exact" }).eq("is_active", true).in("stock_status", filters.status ? [filters.status] : ["available", "coming_soon"]);
  ...
  async getCarById(id: string) { const result = await createPublicServerClient().from("cars").select("id,name,slug,price,currency,stock_status,is_active,brands(name,slug,logo_url),car_models(name,slug),car_images(public_url,alt_text,is_primary,sort_order)").eq("id", id).eq("is_active", true).single(); return { ...result, data: result.data ? normalizeCar(result.data) : null }; }

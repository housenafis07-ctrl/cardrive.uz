...
type NormalizedCar<T extends CarRelations> = Omit<T, "brands" | "car_models"> & { id: string; slug: string; name: string; price: number; currency: string; year: number; stock_status: string; is_featured: boolean; brands: BrandRelation | null; car_models: ModelRelation | null; financing: FinancingSummary };
...
export class CatalogRepository {
  ...
  private async enrichFinancing<T extends CarRelations>(cars: T[] | null): Promise<Array<NormalizedCar<T>>> {
    ...
    return Promise.all(cars.map(async (car) => {
      ...
      const offers = (programs.data ?? []).map((program) => {
        ...
        const payment = monthlyPayment(Number(car.price) * (1 - downPaymentPercent / 100), interestRate, termMonths);
  ...
  async getCars(filters: CatalogQuery) {
    ...
    let query = client.from("cars").select("id,name,slug,price,currency,old_price,year,body_type,fuel_type,transmission,drive_type,engine_volume,stock_status,is_featured,brands!inner(name,slug,logo_url),car_models(name,slug),car_images(public_url,alt_text,is_primary,sort_order,color_id,car_colors(id,name_uz,name_ru,hex_code))", { count: "exact" }).eq("is_active", true).in("stock_status", filters.status ? [filters.status] : ["available", "coming_soon"]);
    if (filters.category) { const category = await client.from("car_categories").select("id").eq("key", filters.category).eq("is_active", true).maybeSingle(); if (category.error) return { ...category, data: null }; if (!category.data) return { data: [], count: 0, error: null }; const items = await client.from("car_category_items").select("car_id").eq("category_id", category.data.id); if (items.error) return { ...items, data: null }; const ids = (items.data ?? []).map((item) => item.car_id); if (!ids.length) return { data: [], count: 0, error: null }; query = query.in("id", ids); }
    ...
    if (filters.q) { const result = await query; if (result.error) return { ...result, data: null }; const normalized = normalizeCars(result.data) ?? []; const enriched = await this.enrichFinancing(normalized); const ranked = enriched.map((car) => ({ car, score: searchRelevance(filters.q!, [car.name, car.brands?.name ?? "", car.car_models?.name ?? "", car.slug]) })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score).map((item) => item.car); return { data: ranked.slice(from, from + PAGE_SIZE), count: ranked.length, error: null }; }

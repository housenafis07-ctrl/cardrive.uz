...
const ORDER_WITH_CAR_SELECT =
  "*, cars(name,slug,car_images(public_url,alt_text,is_primary,sort_order))";

export class OrderRepository {
  async findForCustomer(customerId: string) {
    return createServiceRoleClient()
      .from("orders")
      .select(ORDER_WITH_CAR_SELECT)
  ...
  async create(input: { orderNumber: string; customerId: string; carId: string; purchaseType: PurchaseType; totalAmount: number; currency: string; notes?: string }) {
    return createServiceRoleClient().from("orders").insert({ order_number: input.orderNumber, customer_id: input.customerId, car_id: input.carId, purchase_type: input.purchaseType, total_amount: input.totalAmount, currency: input.currency, notes: input.notes ?? null }).select().single();
  }

  async updateStatus(orderId: string, status: OrderStatus) {
    return createServiceRoleClient().from("orders").update({ status }).eq("id", orderId).select().single();
  }

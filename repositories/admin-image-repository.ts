...
export class AdminImageRepository {
  ...
  async upload(carId: string, file: File) {
    ...
    const existing = await client.from("car_images").select("sort_order").eq("car_id", carId);
    const rows = existing.data ?? [];
    const isFirst = rows.length === 0;
    const nextSortOrder = isFirst ? 0 : Math.max(...rows.map((r) => r.sort_order)) + 1;
    return client
      .from("car_images")
      .insert({
        car_id: carId,
  ...
  async setPrimary(carId: string, imageId: string) {
    const client = this.client();
    await client.from("car_images").update({ is_primary: false }).eq("car_id", carId);
    return client
      .from("car_images")

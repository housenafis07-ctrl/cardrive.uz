...
export class OrderService {
  ...
  async findByIdForCustomer(orderId: string, customerId: string) {
    const order = await this.orders.findByIdForCustomer(orderId, customerId);
    if (!order.data) return { ...order, history: [] as unknown[] };
    const history = await this.orders.findStatusHistory(orderId, customerId);
    return { ...order, history: history.data ?? [] };
  }

  async advanceToConfirmedIfPending(orderId: string, currentStatus: OrderStatus) {
  ...
  async cancelIfOwned(orderId: string, customerId: string) {
    const order = await this.orders.findByIdForCustomer(orderId, customerId);

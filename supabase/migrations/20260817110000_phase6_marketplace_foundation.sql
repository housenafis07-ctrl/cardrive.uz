...
create table if not exists public.car_category_items (
  category_id uuid not null references public.car_categories(id) on delete cascade,
...
insert into public.car_categories(key,name_uz,name_ru,icon,sort_order)
values
('popular','Mashhur avtomobillar','Популярные авто','🔥',10),
('installment','Foizsiz muddatli to‘lov','Беспроцентная рассрочка','💳',20),
('discounts','Issiq chegirmalar','Горячие скидки','🔥',30),
('electric','Elektr avtomobillar','Электрические','⚡',40),
('hybrid','Gibrid avtomobillar','Гибридные','🔋',50),
('premium','Premium avtomobillar','Премиальные авто','💎',60),
('commercial','Tijorat avtomobillari','Коммерческие','🚐',70)
